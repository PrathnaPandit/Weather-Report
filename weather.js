...
async function getweather(city){
    const response= await fetch(`http://api.weatherapi.com/v1/current.json?key=bef7447c9b074da7b8960059220709&q=${city}&aqi=yes`);
...
function processWeather(data){
  ...
  //   conversion('celsius') );
    ...
    fahrenheitbutton.addEventListener( 'click' ,()=>{
      temperatureElement.innerHTML = `${data.current.temp_f}`;
  });
}
